...
describe('Boards REST API', () => {
  ...
  describe('POST /api/boards', () => {
    it('creates a board with nested rounds, categories, and clues', async () => {
      ...
      const response = await authRequest().post('/api/boards').send(payload).expect(201);
    ...
    it('rejects an invalid payload with a 400 error', async () => {
      ...
      const response = await authRequest().post('/api/boards').send({ name: '' }).expect(400);
    ...
    it('rejects a clue with a negative row number', async () => {
      ...
      const response = await authRequest().post('/api/boards').send(payload).expect(400);
    ...
    it('maps a Prisma P2002 unique-constraint violation to 409', async () => {
      ...
      const response = await authRequest().post('/api/boards').send(makeBoardPayload()).expect(409);
  ...
  describe('GET /api/boards', () => {
    it('returns an empty list when no boards exist', async () => {
      ...
      const response = await authRequest().get('/api/boards').expect(200);
    ...
    it('lists all saved boards without nested content', async () => {
      ...
      const payload = makeBoardPayload();
      await authRequest().post('/api/boards').send(payload).expect(201);
  ...
  describe('GET /api/boards/:id', () => {
    it('returns a board with nested rounds, categories, and clues', async () => {
      ...
      const created = await authRequest().post('/api/boards').send(makeBoardPayload()).expect(201);

      const response = await authRequest().get(`/api/boards/${created.body.id}`).expect(200);
    ...
    it('returns 404 for an unknown board id', async () => {
      ...
      const response = await authRequest().get('/api/boards/unknown-id').expect(404);
  ...
  describe('PUT /api/boards/:id', () => {
    it('updates a board, replacing nested content', async () => {
      ...
      const response = await authRequest().put(`/api/boards/${created.body.id}`).send(updatePayload).expect(200);
    ...
    it('returns 404 when updating a non-existent board', async () => {
      ...
      const response = await authRequest().put('/api/boards/non-existent').send(makeBoardPayload()).expect(404);
    ...
    it('maps a Prisma P2002 unique-constraint violation to 409', async () => {
      ...
      const response = await authRequest().put(`/api/boards/${created.body.id}`).send(makeBoardPayload()).expect(409);
    ...
    it('rejects non-positive timer values with a 400 error', async () => {
      ...
      const response = await authRequest().put(`/api/boards/${created.body.id}`).send(payload).expect(400);
    ...
    it('allows clues with empty text and answer to represent blank cells', async () => {
      ...
      const response = await authRequest().put(`/api/boards/${created.body.id}`).send(payload).expect(200);
    ...
    it('returns isComplete true for a fully authored board', async () => {
      ...
      const response = await authRequest().post('/api/boards').send(makeBoardPayload()).expect(201);
  ...
  describe('DELETE /api/boards/:id', () => {
    it('deletes a board and its nested content', async () => {
      ...
      await authRequest().delete(`/api/boards/${created.body.id}`).expect(200);

      const response = await authRequest().get(`/api/boards/${created.body.id}`).expect(404);
      expect(response.body.error).toBe('Board not found');
